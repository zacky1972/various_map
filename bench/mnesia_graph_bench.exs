import VariousMap.MnesiaGraph

defmodule RandomMnesiaGraph do
...
random_edges =
  Enum.map(v_sizes, fn v_size ->
    {time, result} = :timer.tc(fn -> RandomMnesiaGraph.random_edges(v_size) end)
...
inputs =
  ...
  |> Enum.map(fn {v_size, edges} ->
    [v_size, v_size * 2, RandomMnesiaGraph.max_edges(v_size)]
    |> Enum.map(fn e_size ->
      graph = :"graph_#{v_size}_#{e_size}"
      {time, _result} = :timer.tc(fn -> RandomMnesiaGraph.generate(graph, edges, v_size, e_size) end)
      IO.puts("time of create MnesiaGraph: #{time / 1000}msec")
...
Benchee.run(
  %{
    "MnesiaGraph.get" => fn {v_size, _e_size, graph} ->
      get(graph, :rand.uniform(v_size), :rand.uniform(v_size))
    end,
    "MnesiaGraph.put" => fn {v_size, _e_size, graph} ->
